...
%token END 0 "ende der datei"
%left T_INCLUDE T_INCLUDE_ONCE T_EVAL T_REQUIRE T_REQUIRE_ONCE
%token T_INCLUDE      "inkludiere (T_INCLUDE)"
%token T_INCLUDE_ONCE "inkludiere_einmal (T_INCLUDE_ONCE)"
%token T_EVAL         "evaluiere (T_EVAL)"
%token T_REQUIRE      "benötige (T_REQUIRE)"
%token T_REQUIRE_ONCE "benötige_einmal (T_REQUIRE_ONCE)"
%left ','
%left T_LOGICAL_OR
%token T_LOGICAL_OR   "oder (T_LOGICAL_OR)"
%left T_LOGICAL_XOR
%token T_LOGICAL_XOR  "ne_wirklich_oder (T_LOGICAL_XOR)"
%left T_LOGICAL_AND
%token T_LOGICAL_AND  "und (T_LOGICAL_AND)"
%right T_PRINT
%token T_PRINT        "schreibe (T_PRINT)"
%right T_YIELD
...
%token T_INSTANCEOF  "instanzvon (T_INSTANCEOF)"
%right '~' T_INC T_DEC T_INT_CAST T_DOUBLE_CAST T_STRING_CAST T_ARRAY_CAST T_OBJECT_CAST T_BOOL_CAST T_UNSET_CAST '@'
%token T_INC "++ (T_INC)"
%token T_DEC "-- (T_DEC)"
%token T_INT_CAST    "(ganznummer) (T_INT_CAST)"
%token T_DOUBLE_CAST "(fliesskommazahl) (T_DOUBLE_CAST)"
%token T_STRING_CAST "(zeichenkette) (T_STRING_CAST)"
%token T_ARRAY_CAST  "(datenanordnung) (T_ARRAY_CAST)"
%token T_OBJECT_CAST "(objekt) (T_OBJECT_CAST)"
%token T_BOOL_CAST   "(boolesch) (T_BOOL_CAST)"
%token T_UNSET_CAST  "(nicht_mehr_gesetzt) (T_UNSET_CAST)"
%right '['
%nonassoc T_NEW T_CLONE
%token T_NEW       "neu (T_NEW)"
%token T_CLONE     "klon (T_CLONE)"
%token T_EXIT      "ausgang (T_EXIT)"
%token T_IF        "wenn (T_IF)"
%left T_ELSEIF
%token T_ELSEIF    "ansonsten (T_ELSEIF)"
%left T_ELSE 
%token T_ELSE      "sonst (T_ELSE)"
%left T_ENDIF 
%token T_ENDIF     "wennende (T_ENDIF)"
%token T_LNUMBER   "ganzzahl nummer (T_LNUMBER)"
%token T_DNUMBER   "fliesskommazahl nummer (T_DNUMBER)"
%token T_STRING    "identifizierer (T_STRING)"
%token T_STRING_VARNAME "variablen name (T_STRING_VARNAME)"
%token T_VARIABLE  "variable (T_VARIABLE)"
%token T_NUM_STRING "nummer (T_NUM_STRING)"
%token T_INLINE_HTML
%token T_CHARACTER
...
%token T_DO         "tue (T_DO)"
%token T_WHILE      "solange (T_WHILE)"
%token T_ENDWHILE   "solangeende (T_ENDWHILE)"
%token T_FOR        "für (T_FOR)"
%token T_ENDFOR     "fürende (T_ENDFOR)"
%token T_FOREACH    "fürjedes (T_FOREACH)"
%token T_ENDFOREACH "fürjedesende (T_ENDFOREACH)"
%token T_DECLARE    "deklariere (T_DECLARE)"
%token T_ENDDECLARE "deklariereende (T_ENDDECLARE)"
%token T_AS         "als (T_AS)"
%token T_SWITCH     "schalter (T_SWITCH)"
%token T_ENDSWITCH  "schalterende (T_ENDSWITCH)"
%token T_CASE       "fall (T_CASE)"
%token T_DEFAULT    "standard (T_DEFAULT)"
%token T_BREAK      "abbrechen (T_BREAK)"
%token T_CONTINUE   "weitermachen (T_CONTINUE)"
%token T_GOTO       "gehezu (T_GOTO)"
%token T_FUNCTION   "funktion (T_FUNCTION)"
%token T_CONST      "konst (T_CONST)"
%token T_RETURN     "gebezurück (T_RETURN)"
%token T_TRY        "versuche (T_TRY)"
%token T_CATCH      "fange (T_CATCH)"
%token T_FINALLY    "schlussendlich (T_FINALLY)"
%token T_THROW      "werfe (T_THROW)"
%token T_USE        "benutze (T_USE)"
%token T_INSTEADOF  "stattdessen (T_INSTEADOF)"
%token T_GLOBAL     "global (T_GLOBAL)"
%right T_STATIC T_ABSTRACT T_FINAL T_PRIVATE T_PROTECTED T_PUBLIC
%token T_STATIC     "statisch (T_STATIC)"
%token T_ABSTRACT   "abstrakt (T_ABSTRACT)"
%token T_FINAL      "endgültig (T_FINAL)"
%token T_PRIVATE    "privat (T_PRIVATE)"
%token T_PROTECTED  "geschützt (T_PROTECTED)"
%token T_PUBLIC     "öffentlich (T_PUBLIC)"
%token T_VAR        "var (T_VAR)"
%token T_UNSET      "nicht_mehr_gesetzt (T_UNSET)"
%token T_ISSET      "ist_gesetzt (T_ISSET)"
%token T_EMPTY      "leer (T_EMPTY)"
%token T_HALT_COMPILER "__kompilierer_anhalten (T_HALT_COMPILER)"
%token T_CLASS      "klasse (T_CLASS)"
%token T_TRAIT      "charakterzug (T_TRAIT)"
%token T_INTERFACE  "schnittstelle (T_INTERFACE)"
%token T_EXTENDS    "erweiter (T_EXTENDS)"
%token T_IMPLEMENTS "implementiere (T_IMPLEMENTS)"
%token T_OBJECT_OPERATOR "-> (T_OBJECT_OPERATOR)"
%token T_DOUBLE_ARROW    "=> (T_DOUBLE_ARROW)"
%token T_LIST            "liste (T_LIST)"
%token T_ARRAY           "datenanordnung (T_ARRAY)"
%token T_CALLABLE        "aufrufbar (T_CALLABLE)"
%token T_CLASS_C         "__KLASSE__ (T_CLASS_C)"
%token T_TRAIT_C         "__CHARAKTERZUG__ (T_TRAIT_C)"
%token T_METHOD_C        "__METHODE__ (T_METHOD_C)"
%token T_FUNC_C          "__FUNKTION__ (T_FUNC_C)"
%token T_LINE            "__LINIE__ (T_LINE)"
%token T_FILE            "__DATEI__ (T_FILE)"
%token T_COMMENT         "kommentar (T_COMMENT)"
%token T_DOC_COMMENT     "dokument kommententar (T_DOC_COMMENT)"
%token T_OPEN_TAG        "öffnender bezeichner (T_OPEN_TAG)"
%token T_OPEN_TAG_WITH_ECHO "öffnender bezeichner mit echo (T_OPEN_TAG_WITH_ECHO)"
%token T_CLOSE_TAG       "schliessender bezeichner (T_CLOSE_TAG)"
%token T_WHITESPACE      "leerzeichen (T_WHITESPACE)"
%token T_START_HEREDOC   "hierdok start (T_START_HEREDOC)"
%token T_END_HEREDOC     "hierdok ende (T_END_HEREDOC)"
%token T_DOLLAR_OPEN_CURLY_BRACES "${ (T_DOLLAR_OPEN_CURLY_BRACES)"
%token T_CURLY_OPEN      "{$ (T_CURLY_OPEN)"
%token T_PAAMAYIM_NEKUDOTAYIM ":: (T_PAAMAYIM_NEKUDOTAYIM)"
%token T_NAMESPACE       "namensraum (T_NAMESPACE)"
%token T_NS_C            "__NAMENSRAUM__ (T_NS_C)"
%token T_DIR             "__ORDNER__ (T_DIR)"
